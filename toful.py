...
def rth_moment(pmf, support, r, c=0, tol=1e-12, max_iter=10**6):
    """
    Compute the r-th moment of a discrete random variable about c.

    Parameters
    ----------
    pmf : callable
        Function pmf(x) giving P(X = x).
    support : list, range, or str
        - Finite support: list or range of values (e.g., [1,2,3,4,5,6]).
        - Infinite support: pass "infinite" (assumes support starts at 0).
    r : int
        Order of the moment.
    c : float, optional
        Point about which the moment is computed (default=0 → raw moment).
    tol : float, optional
        Tolerance for truncating infinite sums.
    max_iter : int, optional
        Maximum number of iterations when summing infinite series.

    Returns
    -------
    float
        Approximate r-th moment about c.
    """
    ...
    if support == "infinite":
        ...
        while x < max_iter:
            ...
            moment += (x - c)**r * p
            cumulative += p
            # stop when nearly all probability mass is captured
            if 1 - cumulative < tol:
                break
            x += 1
        else:
...
class EnhancedMomentCalculator:
    ...
    def calculate_drv_moment_infinite(func_str: str, range_values: List[float], r: int, a: float, max_iter: int = 10**6, tol: float = 1e-12) -> Tuple[float, dict]:
        """Calculate moments for infinite DRV using the rth_moment function with enhanced precision and convergence analysis"""
        safe_dict = {
        ...
        # Create PMF function from string
        def pmf(x):
            safe_dict['x'] = x
            try:
        ...
        try:
            ...
            # We'll create a simple estimation based on when cumulative probability reaches 1-tol
            cumulative = 0.0
            terms_used = 0
            ...
            # Fallback to partial sum calculation if rth_moment fails
            moment = 0
            terms = []
    ...
    def calculate_drv_moment(func_str: str, range_values: List[float], r: int, a: float, is_infinite: bool = False, max_iter: int = 10**6, tol: float = 1e-12) -> Tuple[float, dict]:
        """Calculate r-th moment for DRV with enhanced precision using rth_moment function"""
        if is_infinite:
        ...
        try:
            ...
            # Fallback to direct calculation if rth_moment fails
            moment = 0
            for x_val in range_values:
                safe_dict['x'] = x_val
                prob = eval(func_str, {"__builtins__": {}}, safe_dict)
...
with st.sidebar:
    ...
    # Set CRV as default by changing the index to 1
                    ...
                    if var_type == "Discrete (DRV)":
                        if is_infinite:
                            mean_val, mean_analysis = EnhancedMomentCalculator.calculate_drv_moment_infinite(
                                prob_func, range_values, 1, 0, max_terms
                            )
                        else:
                            mean_val, mean_analysis = EnhancedMomentCalculator.calculate_drv_moment(
                                prob_func, range_values, 1, 0, is_infinite, max_terms
                            )
                    else:
                ...
                with st.spinner(f"Computing {i}-th moment..."):
                    if var_type == "Discrete (DRV)":
                        moment_val, moment_analysis = EnhancedMomentCalculator.calculate_drv_moment(
                            prob_func, range_values, i, a_value, is_infinite, max_terms
                        )
